...
async function uploadFiles(){var input=document.getElementById('uploadFiles');if(!input.files||!input.files.length){setMsg('Choose one or more files first.',false);return;}var btn=document.getElementById('uploadBtn');var uploadSucceeded=false;btn.disabled=true;resetProgress();try{for(var i=0;i<input.files.length;i++){setMsg('Uploading '+input.files[i].name+' ('+(i+1)+'/'+input.files.length+')...',true);await uploadOne(input.files[i]);}setProgress(100);setMsg('Upload complete.',true);input.value='';updateSelectedFiles();uploadSucceeded=true;loadDirectory(currentPath);}catch(err){setMsg(err.message||'Upload failed',false);}finally{btn.disabled=false;if(!uploadSucceeded)setTimeout(resetProgress,600);}}
