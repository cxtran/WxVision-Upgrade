...
void rebuildSystemInfoModal()
{
    ...
    uint32_t appPartition = running ? running->size : 0;

    uint32_t heapFree = ESP.getFreeHeap();
    uint32_t heapTotal = 327680;
    uint32_t heapUsed = heapTotal - heapFree;
    float heapPct = 100.0f * heapUsed / heapTotal;
    bool hasPsram = psramFound();
    uint32_t psramTotal = hasPsram ? ESP.getPsramSize() : 0;
    uint32_t psramFree = hasPsram ? ESP.getFreePsram() : 0;
    float psramPct = (psramTotal > 0) ? (100.0f * (psramTotal - psramFree) / psramTotal) : 0.0f;
    float flashPct = (appPartition > 0) ? (100.0f * sketchSize / appPartition) : 0.0f;

    uint32_t spiffsTotal = SPIFFS.totalBytes();
    uint32_t spiffsUsed = SPIFFS.usedBytes();
    float spiffsPct = (spiffsTotal > 0) ? (100.0f * spiffsUsed / spiffsTotal) : 0.0f;
    ...
    String lines[] = {
        ...
        "PSRAM: " + String(hasPsram ? String(psramPct, 1) + "% (" + String(psramFree) + " free)" : String("not found")),
    ...
    sysInfoModal.setLines(lines, types, 9);
    sysInfoModal.setKeepOpenOnSelect(true);
    sysInfoModal.setCallback([](bool, int) {});
}
...
void rebuildSdDiagnosticsModal()
{
    ...
    lines[lineCount] = "Mount: " + String(mountOk ? "OK" : "FAIL");
    types[lineCount++] = InfoLabel;
    lines[lineCount] = "Status: " + String(wxv::storage::lastStatus());
    types[lineCount++] = InfoLabel;
    lines[lineCount] = "Type: " + String(wxv::storage::isMounted() ? wxv::storage::cardTypeName() : "NONE");
    types[lineCount++] = InfoLabel;
...
void showSystemModal()
{
    ...
    systemModal.setCallback([](bool accepted, int btnIdx) {
        int action = (btnIdx >= 0) ? btnIdx : (accepted ? systemModal.getSelIndex() : -1);
