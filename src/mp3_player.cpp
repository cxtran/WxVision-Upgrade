...
namespace wxv::audio
{
    namespace
    {
        ...
        uint8_t *allocateStreamBuffer(size_t bytes)
        {
            if (psramFound())
            {
                if (uint8_t *ptr = static_cast<uint8_t *>(heap_caps_malloc(bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)))
                {
                    return ptr;
                }
            }
    ...
    bool startSdMp3(const String &path, int volumePercent)
    {
        ...
        g_streamBuffer.reset(allocateStreamBuffer(kMp3StreamBufferBytes));
        if (!g_streamBuffer)
        {
        ...
        g_decoderWorkspace.reset(allocateStreamBuffer(kMp3DecoderWorkspaceBytes));
        ...
        if (!g_bufferedFile)
        {
            g_lastMp3Status = "buffer init failed";
            cleanupPlayback(true);
            return false;
        }
        ...
        {
            g_lastMp3Status = "id3 init failed";
            cleanupPlayback(true);
            return false;
        }

        g_mp3.reset(new (std::nothrow) AudioGeneratorMP3(g_decoderWorkspace.get(), static_cast<int>(kMp3DecoderWorkspaceBytes)));
        if (!g_mp3)
        {
            g_lastMp3Status = "decoder init failed";
            cleanupPlayback(true);
            return false;
        }
    ...
    void tickSdMp3()
    {
        ...
        const uint32_t sinceLastServiceUs = (g_lastMp3ServiceUs == 0) ? 0 : (burstStartUs - g_lastMp3ServiceUs);
    ...
    {
        g_volumePercent = constrain(volumePercent, 0, 100);
        return !g_out || g_out->SetGain(volumePercentToGain(g_volumePercent));
    }
